...
| DEBT-002 | Testing | 2026-07-04 | `[completar]` | Alto | P1 | Cobertura de tests del backend en el flujo de dinero. **Avance:** (1) firma del webhook MP → `functions/src/lib/mp-signature.ts` con 8 tests; (2) semáforo de salud financiera → `functions/src/lib/financial-health.ts` con 9 tests (desglose IVA/impuestos, caja de sustento y los 6 umbrales del semáforo). Total 17 tests (`npm run test:unit`). **Pendiente:** `unlockWorkerContact` (propiedad/uso único del pago) y validación de cupones — dependen de Firestore, requieren extracción adicional o harness con emulador. | En progreso |
