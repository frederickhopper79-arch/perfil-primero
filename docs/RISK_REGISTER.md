...
| RISK-006 | `functions/src/index.ts` monolítico (~5.400 líneas) con toda la lógica de dinero y **sin tests unitarios de backend**: alto riesgo de regresión en cada deploy. | Media | Alto | `[completar]` | Abierto | Ver DEBT-002. Añadir tests a pagos, desbloqueos y webhooks; modularizar por dominio. |
